 ...
 * toast alerts with a WhatsApp follow-up link.
 * Automatically auto-dismisses alerts 4 seconds after appearance.
 */
...
export default function ReminderAlert({ leads }: ReminderAlertProps) {
  ...
  const checkReminders = useCallback(() => {
    ...
    // Limit to max 3 most urgent so it doesn't flood the UI
    setDueLeads(due.slice(0, 3));
  }, [leads, dismissedIds]);
  ...
  function dismiss(leadId: string) {
    if (timersRef.current[leadId]) {
      clearTimeout(timersRef.current[leadId]);
      delete timersRef.current[leadId];
    }
  ...
  return (
    <div className="fixed top-20 right-6 z-40 flex flex-col gap-3 max-w-sm transition-all duration-300">
      {dueLeads.map((lead) => (
        <div
          key={lead.id}
          className="bg-white border-l-4 border-amber-500 shadow-xl rounded-2xl p-4 animate-slide-up border border-gray-100 transition-all duration-300 hover:shadow-2xl"
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-400 font-mono">auto-closing</span>
              </div>
              <p className="text-xs text-gray-600 mt-1">
                <span className="font-semibold text-gray-800">{lead.raw_name}</span>
                {" · "}
                <span className="font-mono text-gray-500">{lead.sanitized_phone}</span>
              </p>
              <a
                href={generateThankYouURL(lead.sanitized_phone)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 mt-2 text-xs font-semibold text-green-700 hover:text-green-800 transition-colors bg-green-50 px-2.5 py-1 rounded-lg"
              >
